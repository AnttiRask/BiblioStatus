...
### Outcome (done)
Implemented against the real `bibliostatus-app` GCP project (confirmed access and current state — zero existing secrets — before creating anything):

1. Created all 3 Secret Manager secrets (`turso-database-url`, `turso-auth-token`, `carto-api-key`), seeded from the values in the local `secret.R` (values piped directly from an `Rscript` extraction into `gcloud secrets create --data-file=-`, never echoed to the shell/history).
2. Granted `roles/secretmanager.secretAccessor` to `github-deploy@bibliostatus-app.iam.gserviceaccount.com` on all 3 secrets, per plan.
3. **Found a gap in the original plan while executing it**: the plan only considered the *deploying* service account, but Cloud Run resolves `--set-secrets` at container start using the *service's own runtime identity*, which `gcloud run services describe` showed is `884476669514-compute@developer.gserviceaccount.com` (the default compute SA) — not `github-deploy@`. Confirmed with the user before granting, then added the same `secretAccessor` binding for the compute SA on all 3 secrets. Without this, the deploy would have succeeded but the container would have crash-looped trying to resolve secrets it had no permission to read.
4. Updated `.github/workflows/deploy_shiny_app.yml`'s `gcloud run deploy` step from `--set-env-vars ...` to `--set-secrets TURSO_DATABASE_URL=turso-database-url:latest,TURSO_AUTH_TOKEN=turso-auth-token:latest,CARTO_API_KEY=carto-api-key:latest`, exactly per plan. Confirmed `fetch_data.yml` still needs its own `TURSO_DATABASE_URL`/`TURSO_AUTH_TOKEN` GitHub Actions secrets (it's a plain R script outside Cloud Run) — left those GitHub secrets in place, only the deploy workflow's usage changed, as the plan anticipated.

**First deploy attempt failed** (a real production deploy failure, not caught by local testing since this only manifests against the live Cloud Run service's existing state): `ERROR: (gcloud.run.deploy) Cannot update environment variable [TURSO_DATABASE_URL] to the given type because it has already been set with a different type.` Cloud Run refuses to convert an existing plaintext env var to a secret-backed one in the same revision update. The failed deploy did not affect the live service — the previous revision (`bibliostatus-app-00251-j5s`) stayed up and serving throughout (confirmed via a live HTTP 200 check immediately after the failure). Fixed by adding `--remove-env-vars TURSO_DATABASE_URL,TURSO_AUTH_TOKEN,CARTO_API_KEY` before `--set-secrets` in the same deploy command, clearing the old plaintext bindings first.

**Second deploy attempt succeeded**: new revision `bibliostatus-app-00252-fwm` deployed and serving 100% of traffic. Verified via `gcloud run services describe` (parsed as JSON) that all 3 env vars now resolve via `secretKeyRef` pointing at the correct secret names — no plaintext values anywhere in the revision config. Verified via Cloud Run service logs that the container booted cleanly (`Listening on http://0.0.0.0:8080`, then a live `GET 200` for a real request) with zero Turso/Carto/credential-related errors or warnings in the boot or request logs. Both Cloud Build runs for this plan took ~34-37 minutes — confirmed this project's Cloud Build has no image-layer caching configured (both cold-build attempts recompiled every R package from source from the same point in the log, including a from-scratch build immediately after the first one), so this is the expected baseline duration for any deploy here, not specific to this change.
...
### Outcome (done)
Implemented all four sub-items as planned, with two small adjustments:

**10a**: Added `with_turso_fallback()` to `app/www/functions.R`, refactoring `fetch_libraries()`/`fetch_schedules()`/`fetch_library_services()` to call it with closures, preserving each function's exact SQL. Also switched each SQLite fallback closure to `on.exit(dbDisconnect(con))` instead of an explicit trailing `dbDisconnect(con)` call — a minor robustness improvement (connection still closes if the query itself errors) that wasn't in the original plan but fit naturally into the refactor.

**10b**: Added `build_library_popup()` to `app/www/functions.R`, exactly per plan, and switched both `app/server.R` call sites (main map popup, nearest-libraries popup) to use it. One deliberate deviation from "byte-identical": the original main-map popup had a stray double `<br>` before "Status" (cosmetic, not reproduced), and the nearest-libraries popup's field order was "Distance, Status" while the shared helper always emits "Status" before the optional extra line — so that popup's line order is now "Status, Distance" instead. Both are minor, visible-but-harmless content differences inherent to unifying the two structures into one helper; verified the new helper still handles the link/no-link and NA-hours cases correctly (scalar and vectorized) via a standalone R script.

**10c**: Replaced both `warning("Turso write failed - SQLite backup maintained")` calls (libraries+services block, schedules block) with `stop("Turso write failed - SQLite backup was written, but production database was NOT updated. Investigate before the next scheduled run.")`, exactly per plan — the GitHub Actions job will now fail visibly (non-zero exit) instead of silently degrading, while the SQLite backup write still happens first in both cases (the `stop()` comes after).

**10d**: Added all planned named constants to `app/server.R` (`MOBILE_MARKER_RADIUS`, `DESKTOP_MARKER_RADIUS`, `MOBILE_NEAREST_MARKER_RADIUS`, `DESKTOP_NEAREST_MARKER_RADIUS`, `DEFAULT_ZOOM_LEVEL`, `SINGLE_LIBRARY_ZOOM_THRESHOLD`, `MOBILE_NEAREST_COUNT`, `DESKTOP_NEAREST_COUNT`) plus three not explicitly named in the plan but matching its spirit and the "scattered magic numbers" problem statement: `USER_LOCATION_MARKER_RADIUS` (the "You Are Here" marker), and `MOBILE_POPUP_MAX_WIDTH`/`MOBILE_POPUP_MIN_WIDTH`/`DESKTOP_POPUP_MAX_WIDTH`/`DESKTOP_POPUP_MIN_WIDTH` (the popup width values the plan's problem statement explicitly flagged at `server.R:399-400` but whose fix section omitted from the constant list). In `app/ui.R`, added the three named JS timing constants exactly as specified (`STARTUP_GEO_TIMEOUT_MS`, `STARTUP_GEO_FALLBACK_MS`, `FIND_NEAREST_GEO_TIMEOUT_MS`), plus two more for the `maximumAge` values the plan's code sample didn't name but left as bare literals (`STARTUP_GEO_MAX_AGE_MS`, `FIND_NEAREST_GEO_MAX_AGE_MS`) — all values preserved exactly, pure refactor.

**Verified**: `docker compose up --build` succeeds (cache-hit build, no renv changes needed for this plan); app boots and serves HTTP 200 with no new console/log errors beyond the pre-existing benign Turso→SQLite fallback warning during local dev (no local Turso credentials in this container). Full test suite still passes: `FAIL 0 | WARN 2 | SKIP 0 | PASS 31` (run via `Rscript tests/testthat.R`, the correct entrypoint — `testthat::test_dir()` alone skips the app-file sourcing setup and fails to find functions, a testing-harness gotcha worth remembering for next time). `build_library_popup()` verified directly against both call patterns (scalar and vectorized, link/no-link, NA-hours) via a standalone R script inside the container. Browser-based visual verification (Chrome extension) was unavailable in this session — the Claude in Chrome extension reported not connected — so the marker-size/popup-width visual smoke test from the verification plan was not performed; this is a pure constant-rename refactor with no value changes, so risk is low, but a live visual check is recommended before considering this plan fully closed.
...
10. **Plan 8** (secrets hardening) — independent, involves GCP infrastructure changes outside the codebase; needs explicit sign-off before running `gcloud` commands against production. ✅ Done (3 Secret Manager secrets created, IAM granted to both the deploy SA and the Cloud Run runtime SA — the latter was a gap found during execution, not in the original plan; first deploy attempt failed on a Cloud Run env-var-type conflict, fixed with `--remove-env-vars`, second attempt verified live: new revision serving 100% traffic, secret references confirmed via `describe`, clean boot logs with zero credential errors).
